...
const NearbyDiscounts = ({ location }) => {
  ...
  const fetchDiscounts = async () => {
    try {
      setLoading(true);
      const url = `${BASE_URL}productos/descuentos-cercanos?lat=${location.latitude}&lng=${location.longitude}&radio=20`;
      ...
      console.error('Error fetching nearby discounts:', error);
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      ...
      >
        ...
        <View style={styles.infoWrap}>
          <Text style={styles.finalPrice}>{formatPrice(precioFinal)}</Text>
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.sectionTitle}>Descuentos cerca de ti</Text>
      <FlatList
        data={productos}
...
export default NearbyDiscounts;
